...
if(isset($_POST["submit"])) {
    if(isset($_POST["clothesid"])) $clothesid=$_POST["clothesid"];
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
        ...
        $sql = "insert into clothes(clothesid, closetid, timesworn, type, image) values('$clothesid', '$closetid', '0', '$type', '$target_file')";

        $result = $mydb->query($sql);

        if($result==1) {
          echo '<script>console.log("successful insert")</script>';
        } else {
...
<body>
  ...
  <div class="row" id="upload" >
    <form action="create_test.php" method="post" enctype="multipart/form-data">
        ...
        <label>Input A Clothes ID: <br />
          <input type="text" name="clothesid"/>
        </label><br /><br />
