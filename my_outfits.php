<?php
  ...
  //on button click, extract outfit name
  //select outfitid form name
  //delete outfitid from outfitlist and outfit
  for($j=0; $j<$_SESSION['i']; $j++) {
    ...
    if(isset($_POST["name$k"])) {
      ...
      $sql = "select outfitid from outfit where outfit_name='$cname'";
      $result = $mydb->query($sql);
      if($result==1) {
    ...
    if(!empty($oid)){
      $sql = "delete from outfitlist where outfitid='$oid'";
      $result = $mydb->query($sql);

      $sql = "delete from outfit where outfitid='$oid'";
      $result = $mydb->query($sql);
    }


    /* selecting clothesids....
      $sql = "select clothesid from outfitlist where outfitid='$oid'";
      $result = $mydb->query($sql);
      if($result==1) {
        while($row = mysqli_fetch_array($result)){
          $filenames = array($row['clothesid']);
          print_r($filenames);
        }
      }
  ...
  <div class="row">
    <?php
      ...
      foreach($array as $key=>$value) {
        ...
        echo "<input type='hidden' name='name$i' value='$key' />";
